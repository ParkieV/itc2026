...
class GetCommentsByDocAndStageService:
    def __init__(
        self,
    ...
    async def execute(self, doc_id: int, stage_id: int) -> list[Comment]:
        await self._get_pdf_document_service.execute(doc_id)
        await self._get_stage_by_id_service.execute(stage_id)
        return await self._comments_repo.get_by_doc_and_stage_id(doc_id, stage_id)
